...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if sow_sad_state == 1:
        ...
        return sum
    #触发sow sad也要跑完循环，所以增加参数sow_sad_state
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    #opponent_score // 10 % 10是为了获取其十位数，不能默认分数小于100
    # END PROBLEM 2
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def always_roll_number(score, opponent_score):
        return n
    return always_roll_number
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for player_score in range(goal):
        for opponent_score in range(goal):
            ...
            if strategy(player_score, opponent_score) != expected:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range(1, 11):
        current_avg = averaged_roll(num_rolls, dice)
        if current_avg > max_avg or (current_avg == max_avg and num_rolls < best_roll):
            max_avg = current_avg
            best_roll = num_rolls
    return best_roll
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    averaged_roll = make_averaged(roll_dice, times_called=100)
    max_avg = 0
    best_roll = None
    sus_gain = sus_update(0, score, opponent_score) - score
    for num_rolls in range(4, 7):
        current_avg = averaged_roll(num_rolls)
        if current_avg > max_avg or (current_avg == max_avg and num_rolls < best_roll):
            max_avg = current_avg
            best_roll = num_rolls
    if sus_gain > max_avg:
